...
- **$SCRATCH's own trading tax split (final, 2026-07-19): 80% game rewards, 10% ops, 10% burn.** Mechanically: `dividendBps=0`, `deflationBps=1000` (10%, Flap-native automatic buyback-and-burn — shrinks supply, confirmed mechanism), `mktBps=9000` (90%) routes to `TokenTaxRouter.sol`, a single fixed beneficiary (not per-holder) which then splits *that* ~11.11%/~88.89% ops/prize-pools — works out to 10%/80% of the *total* tax once you account for the 90% it actually receives. The 80% share lands in ScratchCore's prize pools via `ScratchCore.fundPools()`, split 50/50 between `instantPool` and `jackpotPot`. This taxes $SCRATCH's own buy/sell volume — separate from the ticket rake below, do not conflate the two. Trading $SCRATCH literally funds the game and shrinks its own supply; it does not pay its own holders.
- `lpBps` is deliberately left at 0. Its actual mechanics aren't documented anywhere in Flap's own docs (checked the Portal launch reference, Tax Token V2/V3 mechanics, the Robinhood integration guide, protocol economics, and pre-bond tax pages) — not safe to rely on for a permanent, irreversible launch parameter. An initial idea to route 10% toward "thickening LP" was dropped for the deflation/burn mechanism instead, since burn is fully documented and verified.
...
- Tax tokens on Robinhood Chain must deploy via CREATE2 with a salt producing an address ending in `7777` (Flap's own requirement, confirmed in their Robinhood Chain Integration Guide) — `LaunchScratchToken.s.sol` brute-forces this on-chain via `vm.computeCreate2Address`, replicating Flap's documented off-chain algorithm. An earlier draft (inherited from a different project) used a plain random salt with no vanity targeting and would have reverted on a real launch attempt.
...
(Superseded three times now: the original plan had $SCRATCH's tax feeding the jackpot directly via a custom 50/25/25 split, plus hold-based utility features and a USDG payment option — dropped for Flap's native dividend mechanism, simpler and already-live infrastructure. Then a 90%-to-holders dividend plan was dropped in favor of routing that revenue into the game's own prize pools instead, since the actual goal was never to reward passive holders. Then an 80/10/10-with-LP idea was simplified to 80/10/10-with-burn once `lpBps`'s mechanics turned out to be undocumented — see conversation notes 2026-07-19. Tickets are also ETH-only now, not ETH/USDG — see `src/ScratchCore.sol`, which is also flat-multiplier rather than the percent-of-pool model §2 below still describes; that section is stale too and not fixed as part of this edit.)
