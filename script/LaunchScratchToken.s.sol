...
/// decided 2026-07-19. dividendBps is 0; the entire tax routes through
/// mktBps to a single beneficiary, TokenTaxRouter.sol, which sweeps it
/// 10% ops / 90% straight into ScratchCore's prize pools (fundPools(),
/// split 50/50 instant/jackpot there). This is the opposite of Flap's more
/// common "reward holders" pattern — here, trading $SCRATCH funds the game
/// instead of paying dividends. Nothing about the token itself is custom;
/// only where its tax revenue is aimed.
///
...
///   DIVIDEND_BPS (0)             -> nothing; holders get no dividend
///   MKT_BPS      (10000 = 100%)  -> TOKEN_TAX_ROUTER, which then splits
///                                   10% ops / 90% ScratchCore prize pools
///
/// Env:
...
///   MKT_BPS                  default 10000 (100% of tax -> TOKEN_TAX_ROUTER)
...
contract LaunchScratchToken is Script {
    ...
    function run() external returns (address token) {
        ...
        LaunchConfig memory cfg = LaunchConfig({
            ...
            mktBps: uint16(vm.envOr("MKT_BPS", uint256(10_000))),
